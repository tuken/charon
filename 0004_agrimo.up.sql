ALTER `orgs` ADD COLUMN `latitude` DECIMAL(10,7) DEFAULT NULL COMMENT '緯度' AFTER `address`;

ALTER `orgs` ADD COLUMN  `longitude` DECIMAL(10,7) DEFAULT NULL COMMENT '経度' AFTER `latitude`;

UPDATE `orgs` SET `latitude` = 33.1234567, `longitude` = 132.1234567;

ALTER `orgs` MODIFY COLUMN `latitude` DECIMAL(10,7) NOT NULL COMMENT '緯度';

ALTER `orgs` MODIFY COLUMN `longitude` DECIMAL(10,7) NOT NULL COMMENT '経度';
